...
### naive_gather.c
I start by implementing a Naive strategy to assess an initial baseline using p2p communication, a naive approach is that each slave issues a blocking send to root. Root issues `np` blocking recv to receive all the data in order.